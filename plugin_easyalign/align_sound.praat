...
if language$="lt"
  chars_to_ignore$=replace$(chars_to_ignore$,"'","",0)
  chars_to_ignore$=replace$(chars_to_ignore$,"-","",0)
endif
...
if language$="lt"
  Insert string: 0, "HNET:ALLOWXWRDEXP = TRUE"
  Insert string: 0, "HNET:ALLOWCXTEXP = TRUE"
  Insert string: 0, "HNET:FORCECXTEXP = TRUE"
  Insert string: 0, "HNET:CFWORDBOUNDARY = TRUE"
endif
...
for i from 1 to n
 ...
 #PROCESS the others
   ...
   if language$="nl"
     call checknl 'l$'
   elsif language$="fra" or language$="sw" or language$="en" or language$="en2" or language$="nan" or language$="spa" or language$="slk" or language$="porbra"
   ...
   if isgood!=0
     ...
     elsif language$="fra" or language$="sw" or language$="en" or language$="en2" or language$="nan" or language$="spa" or language$="slk" or language$="porbra" or language$="lt"
	...
	select Table 'language$'
	  ...
	  call addwordtodct 'final_space' 'w$'
        ...
        if (allow_elision=1) and (index(wout$,"@")>0)
          wout$=replace$(wout$," @","",0)
          fileappend "tmp/'basename$'_'i'.dct" 'w1$''tab$''wout$' 'newline$'
        endif
...
if make_syllable_tier
  if language$="lt"
    execute lang/lt/ltsyllabify.praat
  else
      ...
      if w$=="sp"
        ...
        if language$="lt"
          ...
          if cd_right>0
            w$=left$(w$,cd_right-1)
if .verbose=1
    printline CDRIGHT 'cd_right' 'w$'
endif
          endif
          cd_left=rindex(w$,"-")
          if cd_left>0
            wlen=length(w$)
            w$=right$(w$,wlen-cd_left)
if .verbose=1
    printline CDLEFT 'cd_left' 'w$'
endif
          endif
        endif
...
procedure addwordtodct .final_space .w$
    ...
    if language$=="lt"
      call convertsampatohtkseparated '.w$' |- 
      printline 'convertsampatohtkseparated.wout$'
      .wout$=replace$(convertsampatohtkseparated.wout$," ","",1)
    else
