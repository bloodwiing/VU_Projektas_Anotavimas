...
if language$=="fra" or language$=="en" or language$="en2"
  ...
  Text writing preferences... try ISO Latin-1, then UTF-16
  Text reading preferences... try UTF-8, then ISO Latin-1
   printline 'langDir$'transcrLUSS.exe 'langDir$' 'tmpDir$'t.txt 'tmpDir$'tout.txt
  for i to ns
    select sID
    s$=Get string... 'i'
    #pause 's$'
    if s$="_" or s$="" or left$(s$,1)="%"
      ...
      filedelete tmp/t.txt
      fileappend tmp/t.txt 's$'
      system 'langDir$'transcrLUSS.exe 'langDir$' 'tmpDir$'t.txt 'tmpDir$'tout.txt
