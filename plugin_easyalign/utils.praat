...
procedure convertsampatohtkseparated .w$ .separator$
  .l = length(.w$)
  .wout$=""
  for .ii to length(.w$)
    .endcut = .l-.ii+1
    .search$ = right$(.w$,.endcut)

    for .s to length(.separator$)
      .char$ = mid$(.separator$,.s,1)
      .newcut = index(.search$,.char$)-1

      if .newcut!=-1 and .newcut<.endcut
        .endcut = .newcut
      endif
      printline '.char$' '.newcut' '.endcut'
    endfor

    .search$ = left$(.search$,.endcut)
    .searchl = length(.search$)
    .c2$ = .search$
    ...
    if .c2$!=""
      ...
      if .convrow!=0
        .c2$=Get value... '.convrow' htk
        printline '.c2$' FOUND MATCH '.search$' '.ii'
        .ii=.ii+.searchl-1
      endif
